// ============================================================
// In-Memory Cinema Database — 150 cinemas across India
// No PostgreSQL required. Works entirely in-memory.
// ============================================================
...
function buildCinemas(): Cinema[] {
  ...
  // Distribute across cities — bigger cities get more cinemas
  ...
  for (const cityInfo of CITIES) {
    ...
    for (let i = 0; i < count && cinemaIdx < 150; i++) {
      ...
      // Slight lat/lng offset so cinemas spread around the city
      const latOffset = (rng() - 0.5) * 0.1;
      const lngOffset = (rng() - 0.5) * 0.1;

      const screenCount = 1 + Math.floor(rng() * 2); // 1-2 screens
...
// Singleton — generated once at server startup
...
// Track booked seats in-memory so they persist across requests during the session
const bookedSeatsMap = new Map<string, Set<string>>(); // showtimeId -> Set<seatId>
...
export function generateShowtimes(movieId: string, userLat?: number, userLng?: number): CinemaShowtime[] {
  ...
  for (const cinema of cinemas) {
    for (const screen of cinema.screens) {
      ...
      for (let day = 0; day < 5; day++) {
        // Each screen shows 2-3 times per day for this movie
        ...
        for (const hour of dayTimes) {
          ...
          // Pre-book some seats deterministically for realism
          const bookedSet = getBookedSeats(showtimeId);
          if (bookedSet.size === 0) {
            // First time: seed some random bookings (about 20-40% of seats)
  ...
  // Return top 40 showtimes (from nearest cinemas) to keep response manageable
